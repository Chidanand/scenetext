...
load aspect_ratio;
global ab_length_before;
ab_length_before = 0;

dfs={'alpha',.8,'radx',3,'rady',1,'rads',2,'cap',500,'mpw',5,...
  'wthr',Inf,'wolap',.5,'ap_mean',ap_mean,'ap_var',ap_var};
[prm.alpha,prm.radx,prm.rady,prm.rads,prm.cap,prm.mpw,prm.wthr,prm.wolap,prm.ap_mean,prm.ap_var] = getPrmDflt(varargin,dfs,1);
  ...
  for pari=1:length(leaf.kids), par_t=leaf.kids(pari);
    ...
    for ip=1:size(parbbs,1)
      ...
      for j=1:length(isectIdx), ic=isectIdx(j);
        cost1=cost(chibbs(ic,:),parbb,prm.alpha,prm.cap,prm.ap_mean,prm.ap_var);
...
if(lex(chi_t).isWord),
  ...
  for i=1:size(rootbbs,1)
    rootbb=rootbbs(i,:); [~,u]=cost(rootbb,rootbb,0,prm.cap,prm.ap_mean,prm.ap_var);
...
for pari=1:length(lex(chi_t).kids), par_t=lex(chi_t).kids(pari);
  ...
  for ip=1:size(parbbs,1)
    ...
    for j=1:length(isectIdx), ic=isectIdx(j);
      ...
      cost1=cost(chibbs(ic,:),parbb,prm.alpha,prm.cap,prm.ap_mean,prm.ap_var)+BvalC(ic);
      if(cost1<minb), minb=cost1; mini=ic; end        
    end
    Bval1(ip,1)=minb; Barg1(ip,1)=mini;
  end
...
function [c,u,p]=cost(chibb,parbb,alpha,cap,ap_mean,ap_var)
...
ycent=parbb(1,2)+.5*parbb(1,4);
xdist=abs(chibb(1,1)-xcent)/max(chibb(1,3),parbb(1,3));
...
% cen1 = [chibb(1,1)+.5*chibb(1,3) chibb(1,2)+.5*chibb(1,4)];
% cen2 = [parbb(1,1)+.5*parbb(1,3) parbb(1,2)+.5*parbb(1,4)];
% ab_length = norm(cen1-cen2,2);

% if ab_length_before == 0
%     d_dist = 0;
% else
%     d_dist = abs(ab_length-ab_length_before) / max(ab_length,ab_length_before);
%     ab_length_before = abs(ab_length-ab_length_before);
% end
% % d_dist = 0;
% ang_dist = 1 - cen1*cen2' / ( norm(cen1,2) * norm(cen2,2) );
% if ang_dist < 0
%     ang_dist = 2;
% end
