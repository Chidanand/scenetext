...
paramSets={{'icdar','charEasy','icdar',5000,1}};
...
for p=1:length(paramSets)
  ...
  RandStream.getDefaultStream.reset();
  [ferns,yh]=fernsClfTrain(x,y,struct('S',S,'M',M,'thrr',thrr,'bayes',1));
  ...
  if(~exist(fullNewBgD,'dir')),mkdir(fullNewBgD); end  
  ...
  % bootstrap
  for f=1:length(files), fprintf('image %i\n',f);
    ...
    [bbs,P]=bbGt('sampleData',I,{'bbs',bbs,'n',maxn,'dims',[100 100],'ibbs',...
      gtBbs1,'thr',.01});
